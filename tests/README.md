...
Core tests take longer than any other WolfCoin tests, due to the high amount of computational work involved in validating core components.
...
To run only WolfCoin's core tests (after building):
...
To run only WolfCoin's crypto tests (after building):
...
To run only WolfCoin's hash tests (after building):
...
To run only WolfCoin's performance tests (after building):
...
To run only WolfCoin's unit tests (after building):
